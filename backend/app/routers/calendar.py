...
def google_callback(
    ...
    calendar_url = f"{root}/calendar/"

    try:
        if error:
            return RedirectResponse(f"{calendar_url}?google_error={urllib.parse.quote(error)}")

        if not code or not state:
            return RedirectResponse(f"{calendar_url}?google_error=missing_code_or_state")

        user_id = _parse_state(state)
        user = db.get(User, user_id)
        if not user:
            return RedirectResponse(f"{calendar_url}?google_error=user_not_found")
        ...
        if not token_resp.is_success:
            ...
            return RedirectResponse(f"{calendar_url}?google_error={urllib.parse.quote(detail)}")

        access_token = token_resp.json().get("access_token", "")
        if not access_token:
            return RedirectResponse(f"{calendar_url}?google_error=no_access_token")
        ...
        if not events_resp.is_success:
            return RedirectResponse(f"{calendar_url}?google_error=fetch_failed_{events_resp.status_code}")
        ...
        return RedirectResponse(f"{calendar_url}?google_import={created}")

    except HTTPException:
        raise
    except Exception as exc:
        return RedirectResponse(f"{calendar_url}?google_error={urllib.parse.quote(str(exc)[:200])}")
