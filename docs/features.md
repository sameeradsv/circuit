...
- **AI task defaults** — `POST /api/ai/suggest-task` infers duration, effort, focus type, cognitive/priority/energy fields, reminders, schedule hints, and a tiny step from the actual event/task name. Explicit user hints such as `#work`, `30m`, or a parsed date/time override the suggestion. `POST /api/tasks` also fills any omitted fields as a server-side safety net.
