...
export function TaskPrioritySection({ merged, set }: TaskSectionProps) {
  return (
    <section className="space-y-3">
      ...
      <Slider label="Cost of delay" value={merged.consequence_of_delay ?? 0.3} onChange={(v) => set("consequence_of_delay", v)} hint="Impact if this is pushed back. Raises the score when delay has real consequences." />
      <Slider label="Unlocks next steps" value={merged.momentum_value ?? 0.5} onChange={(v) => set("momentum_value", v)} hint="How much finishing this opens the way for other work or makes the next move easier." />
    </section>
  );
}
